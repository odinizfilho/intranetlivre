    ...
    <div>
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('posts.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
			@method('PUT')

                        <div class="mb-4">
                            <label for="title" class="block text-gray-700 text-sm font-bold mb-2">{{ __('Título') }}</label>
                            <input type="text" name="title" id="title" class="border border-gray-400 rounded w-full py-2 px-3"  value="{{ $blog->title }}" required>
                        </div>

                        <div class="mb-4">
                            <label for="content" class="block text-gray-700 text-sm font-bold mb-2">{{ __('Conteúdo') }}</label>
                            <div id="">
                                <textarea type="text" name="content" id="content" class="ckeditor border border-gray-400 rounded w-full py-2 px-3" required>{{ $blog->content }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="categories[]" id="categories" multiple class="border border-gray-400 rounded w-full py-2 px-3">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ in_array($category->id, $blogCategories) ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
