                    ...
                    <form action="{{ route('colaboradores.store') }}" method="POST">
                            ...
                            <label for="matricula"
                                class="block text-gray-700 text-sm font-bold mb-2">{{ __('Matricula do Colaborador') }}</label>
                                    ...
                                    @foreach ($usuarios as $usuario)
                                        <option value="{{ $usuario->matricula }}">{{ $usuario->name }}</option>
                                    @endforeach
                                </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="cod_unidade" id="unidade" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                @foreach ($unidades as $unidade)
                                    <option value="{{ $unidade->cod_unidade }}">{{ $unidade->nome }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="tel" name="telefone" id="telefone"
                                class="border border-gray-400 rounded w-full py-2 px-3" value="{{ old('telefone') }}"
                                required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="ramal" id="ramal"
                                class="border border-gray-400 rounded w-full py-2 px-3" value="{{ old('ramal') }}">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="cod_cargo" id="cargo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                @foreach ($cargos as $cargo)
                                    <option value="{{ $cargo->cod_cargo }}">{{ $cargo->nome }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            <label for="Cargo"
                                class="block text-gray-700 text-sm font-bold mb-2">{{ __('Setor') }}</label>
                            <select name="cod_setor" id="setor" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                @foreach ($setores as $setor)
                                    <option value="{{ $setor->cod_setor }}">{{ $setor->nome }}</option>
                                @endforeach
                            </select>
                        </div>
